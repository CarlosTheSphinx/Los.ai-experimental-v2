...
// Underwriting Reports — generated by the two-agent underwriting orchestration
export const underwritingReports = pgTable("underwriting_reports", {
  id: serial("id").primaryKey(),
  projectId: integer("project_id")
    .references(() => projects.id, { onDelete: "cascade" })
    .notNull(),
  status: varchar("status", { length: 50 }).default("pending").notNull(), // pending | running | complete | failed
  dealSummary: jsonb("deal_summary"),       // output from underwriting_extractor agent
  report: jsonb("report"),                  // output from underwriting_analyst agent
  overallLikelihood: varchar("overall_likelihood", { length: 20 }), // high | medium | low | unlikely
