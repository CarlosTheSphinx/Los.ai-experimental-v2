...
-   **Loan Strategy Dimension**: Each fund has a `loanStrategy` field (Bridge/Permanent/Both/null) parsed from specialty knowledge entries. The fund matcher pre-filters by loan strategy (Bridge deals only match Bridge or Both funds) and includes strategy in the AI context. The fund form UI shows strategy toggle buttons. Bulk import supports `loanStrategy` column mapping. A backfill endpoint (`POST /api/commercial/funds/backfill-strategy`) re-parses all specialty entries to populate strategies.
