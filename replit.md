...
-   **Quote PDF Generator**: Server-side PDF generation using `pdf-lib` with customizable templates. Admins manage PDF templates in Settings > Quote PDFs (company name, tagline, colors, sections, fields, disclaimer). Templates stored in `quote_pdf_templates` table with jsonb config. Two download endpoints: `GET /api/quotes/:id/pdf` for saved quotes, `POST /api/pricing/pdf` for unsaved pricing results. Flexible field resolution handles dynamic form keys via normalized alias matching. Generator utility at `server/pdf/quoteGenerator.ts`.
