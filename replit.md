...
-   **Configurable Broker Welcome Email**: Sends a customizable welcome email when a broker registers (standard or Google OAuth). Template (subject + HTML body) stored in system settings keys `broker_welcome_email_enabled` and `broker_welcome_email_template` with merge tags ({{firstName}}, {{fullName}}, {{portalLink}}, {{supportEmail}}). Admin UI in Settings → Email Templates tab. Implementation in `server/email.ts` (`sendBrokerWelcomeEmail`).
