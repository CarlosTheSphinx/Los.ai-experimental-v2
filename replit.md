...
- **Key Features**:
    ...
    - **Borrower Portal**: A public, token-based portal for borrowers to view loan progress, required documents checklist grouped by pipeline stage with upload capability (status: pending/uploaded/approved/rejected with rejection reasons), and activity timeline. Documents tab is the default view. Uploads go through object storage with presigned URLs and auto-sync to Google Drive.
