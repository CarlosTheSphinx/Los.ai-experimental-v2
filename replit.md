...
Full tenant isolation across the platform. Each lender (admin user) is a tenant. Key tables have a `tenantId` column: `projects`, `partners`, `admin_tasks`, `pricing_requests`, `system_settings`. The `loan_programs` and `credit_policies` tables use `created_by` for ownership. Tenant resolution is handled by `server/utils/tenant.ts` — `getTenantId(user)` returns `null` for super_admin (cross-tenant), the admin's own ID for root admins, or traces the `invitedBy` chain for invited users. All admin API endpoints (pipeline, deals, dashboard stats, settings, partners, tasks) filter by tenant. System settings use a compound `(settingKey, tenantId)` constraint for per-tenant configuration with fallback to global defaults. A one-time backfill script (`server/utils/backfill-tenants.ts`) ran on startup to assign tenant IDs to existing data. Super admins have a dedicated management interface at `/admin/platform` with full visibility into all tenants, and a tenant detail page at `/admin/platform/tenants/:tenantId` with tabs for overview, users, deals, programs, and settings — including ability to toggle tenant active/inactive, enable/disable users, and reset passwords.
